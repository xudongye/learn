 ...
 */
public class MsgUserRelationKeyGenerator implements IdentifierGenerator, Configurable {

    private final static Logger log = LoggerFactory.getLogger(MsgUserRelationKeyGenerator.class);
    ...
    public synchronized Serializable generate(SessionImplementor arg0, Object arg1)
        ...
        try {
            ...
            synchronized (lock){
                lockPstmt = con.prepareStatement("lock tables MsgUserRelationIdentifier write");
                lockPstmt.execute();

                queryPstmt = con.prepareStatement("select count(*) from MsgUserRelationIdentifier where date = ?");
                ...
                insertPstmt = con.prepareStatement("insert into MsgUserRelationIdentifier (dailySequence, date) values (?,?)");;
